...
struct IBB_ModProject
{
    ...
    std::wstring BuildOutputDir;                // 编译输出目录
    std::string BuildLog;                       // 编译日志
    std::unordered_map<std::string, std::vector<std::wstring>> CompiledIniFiles; // 编译产物INI按类型分组
    ...
    static constexpr const char* ModProjKey = "ModProjPath";

    // 二进制元数据尾缀（替代 _METADATA_ 模块）
    static void WriteMetaTail(std::vector<BYTE>& data, const std::unordered_map<std::string, std::string>& kv);
    static std::unordered_map<std::string, std::string> ReadMetaTail(const std::vector<BYTE>& data);

    // iproj ModProjPath 操作
    static void AddIprojModProjPath(const std::wstring& iprojPath, const std::wstring& modprojPath);
    static void RemoveIprojModProjPath(const std::wstring& iprojPath, const std::wstring& modprojPath);
    static std::vector<std::wstring> GetIprojModProjPaths(const std::wstring& iprojPath);
