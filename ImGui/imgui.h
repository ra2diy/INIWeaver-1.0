...
{
    double                                   x, y;
    constexpr dImVec2()                      : x(0.0F), y(0.0F) { }
    constexpr dImVec2(double _x, double _y)  : x(_x), y(_y) { }
    constexpr dImVec2(const ImVec2 _m)       : x(_m.x), y(_m.y) {}
    constexpr dImVec2(const dImVec2&) = default;
    //constexpr dImVec2(dImVec2&&) = delete;
    double  operator[] (size_t idx) const    { IM_ASSERT(idx <= 1); return (&x)[idx]; }    // We very rarely use this [] operator, the assert overhead is fine.
    double& operator[] (size_t idx)          { IM_ASSERT(idx <= 1); return (&x)[idx]; }    // We very rarely use this [] operator, the assert overhead is fine.
    operator ImVec2() { return ImVec2{ (float)x,(float)y }; }
    dImVec2& operator=(ImVec2 _m)            { x = (double)_m.x, y = (double)_m.y; return *this; }

    double max() { return x > y ? x : y; }
    double min() { return x < y ? x : y; }

#ifdef D_IM_VEC2_CLASS_EXTRA
    D_IM_VEC2_CLASS_EXTRA     // Define additional constructors and implicit cast operators in imconfig.h to convert back and forth between your math types and ImVec2.
#endif
};
...
namespace ImGui
{
    ...
    IMGUI_API bool          SmallButton(const char* label, const ImVec2& size = ImVec2(0, 0));// button with FramePadding=(0,0) to easily embed within text
    IMGUI_API bool          SmallButtonAlignLeft(const char* label, const ImVec2& size = ImVec2(0, 0));
    ...
    IMGUI_API bool          BeginComboEx(const char* label, const char* preview_value, ImGuiComboFlags flags, bool extra_active, ImVec2 popup_pos);
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoClamping             = 1 << 21,  // No window clamping so you can freely move windows
